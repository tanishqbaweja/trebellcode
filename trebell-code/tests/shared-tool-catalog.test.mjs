...
test("shared Trebell tool catalog owns unique schemas and policy metadata",()=>{
  ...
  assert.deepEqual(namespaceNames,["trebell_browser","trebell_computer","trebell_device","trebell_source_control","trebell_delegate"]);
  for(const namespace of SHARED_TOOL_NAMESPACE_CATALOG){
    ...
    for(const item of namespace.tools){
      ...
      assert.equal(typeof item.policy.idempotent,"boolean");assert.equal(typeof item.policy.externalSideEffect,"boolean");assert.equal(typeof item.policy.asyncSafe,"boolean");
...
test("dynamic tool exposure is capability-driven",()=>{
  ...
  const enabled=sharedDynamicToolNamespaces({browser:true,computer:true,device:true,sourceControl:true,delegation:true});
  assert.deepEqual(enabled.map(item=>item.name),["trebell_browser","trebell_computer","trebell_device","trebell_source_control","trebell_delegate"]);
