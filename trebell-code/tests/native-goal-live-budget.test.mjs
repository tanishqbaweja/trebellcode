...
test("Native goal wall-time budget aborts an in-flight provider before the turn can overrun",async()=>{
  ...
  try{
    ...
    const started=Date.now(),turn=(await h.rpc.request("turn/start",{threadId:thread.id,model:"model-a",modelProvider:"agentrouter",input:[{type:"text",text:"slow work"}]})).turn;const completed=await h.rpc.waitFor(message=>message.method==="turn/completed"&&message.params?.turn?.id===turn.id,5000),elapsed=Date.now()-started;
    assert.equal(completed.params.turn.status,"failed");assert.match(completed.params.turn.error?.message||"",/wall-time budget exhausted/i);assert.equal(providerAborted,true);assert.ok(elapsed<2500,`wall-time budget took ${elapsed}ms`);
