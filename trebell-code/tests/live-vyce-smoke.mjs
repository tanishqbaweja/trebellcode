...
const guard=createLiveSmokeGuard({provider:"vyceai",model:"deepseek-v4.1",runtime:"direct",maxTurns:1,timeoutMs:90_000});
if(!apiKey){
...
try{
  ...
  guard.consumeTurn("Vyce direct inference");
  const result=await guard.withTimeout(manager.directChat("vyceai",{
    model:"deepseek-v4.1",
  ...
  console.log(JSON.stringify({ok:true,provider:"vyceai",model:"deepseek-v4.1",catalogSource:catalog.source,fingerprint:guard.fingerprint()},null,2));
}finally{
