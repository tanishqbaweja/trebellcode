...
test("runtime capabilities describe adapter behavior without pretending unsupported features exist",()=>{
  ...
  assert.equal(native.mcpInjection,true);assert.equal(native.compaction,true);assert.equal(native.delegation,true);assert.equal(native.steering,true);assert.equal(native.backgroundProcesses,true);
