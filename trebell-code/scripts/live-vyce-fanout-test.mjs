...
try{
  ...
  const launches=models.map(async(model,index)=>{
    ...
    const turn=await rpc(ws,nextId++,"turn/start",{threadId,model,cwd:worktree,approvalPolicy:"never",sandboxPolicy:{type:"readOnly",networkAccess:false},input:[{type:"text",text:"Reply with exactly TREBELL_FANOUT_OK and nothing else. Do not use tools.",text_elements:[]}]});if(!turn.turn?.id)throw new Error(model+" did not return a turn id");
  ...
  console.log(JSON.stringify({ok:true,provider:"vyceai",results:results.map(({model,threadId,worktree,reply})=>({model,threadId,isolatedWorktree:worktree!==repo,reply}))},null,2));
