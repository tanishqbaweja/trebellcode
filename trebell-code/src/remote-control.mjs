...
function mobileHtml(){
  return [
    ...
    "<section class=\"card\"><div class=\"row stack\"><input id=\"token\" class=\"grow\" type=\"password\" placeholder=\"Device session token\"><button id=\"connect\" class=\"primary\">Connect</button></div><div id=\"status\" class=\"status\"></div><div id=\"scopeStatus\" class=\"status\"></div></section>",
    ...
    "<section class=\"card\"><div class=\"row stack\"><select id=\"permission\"><option value=\"supervised\">Supervised</option><option value=\"auto\">Auto</option><option value=\"full\">Full access</option><option value=\"read-only\">Read only</option></select><select id=\"collaborationMode\" hidden title=\"Codex collaboration mode\"></select><input id=\"cwd\" placeholder=\"Workspace path\"></div><textarea id=\"prompt\" placeholder=\"Ask Trebell to do something…\"></textarea><div class=\"row\"><button id=\"stop\">Stop</button><button id=\"send\" class=\"primary grow\">Send</button></div><pre id=\"transcript\" class=\"output\"></pre></section>",
    ...
    "</main><script>",
    "(function(){",
    "const $=id=>document.getElementById(id);let ws=null,pending=new Map(),seq=1,activeThread=null,activeTurn=null,statusData=null,collaborationModes=[];",
    ...
    "function hasScope(scope){return Array.isArray(statusData?.remoteScopes)&&statusData.remoteScopes.includes(scope)}",
    "function scopeControl(id,allowed,scope){const control=$(id);if(!control)return;control.disabled=!allowed;if(!allowed)control.title='This paired device does not have '+scope+' access.';else if(control.title?.startsWith('This paired device does not have '))control.removeAttribute('title')}",
    "function applyScopeUi(){const scopes=Array.isArray(statusData?.remoteScopes)?statusData.remoteScopes:[];scopeControl('refreshThreads',hasScope('threads:read'),'thread read');for(const id of ['newThread','permission','cwd','prompt','stop','send'])scopeControl(id,hasScope('threads:write'),'thread write');scopeControl('environment',hasScope('environments:read'),'environment read');scopeControl('probe',hasScope('environments:read'),'environment read');scopeControl('command',hasScope('environments:execute'),'environment execute');scopeControl('runCommand',hasScope('environments:execute'),'environment execute');$('scopeStatus').textContent=scopes.length?'Granted: '+scopes.join(' · '):'No remote capabilities granted.'}",
    ...
    "async function connect(){if(pairToken){const paired=await fetch('/api/pair',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({token:pairToken,name:navigator.userAgentData?.platform||navigator.platform||'Remote browser'})}).then(async r=>{const d=await r.json();if(!r.ok)throw new Error(d.error||'Pairing failed');return d});pairToken='';$('token').value=paired.token;localStorage.setItem('trebellRemoteSession',paired.token);localStorage.removeItem('trebellRemoteToken')}const t=token();if(!t)return alert('Open a pairing link from your Trebell host or enter a device session token');localStorage.setItem('trebellRemoteSession',t);statusData=await api('/api/status');applyScopeUi();const harness=statusData.agentRuntime||'codex';const providerLabel=harness==='codex'?(statusData.provider||'freebuff'):harness;$('status').textContent='Desktop '+statusData.version+' · '+providerLabel+(statusData.providerReady?' ready':' not ready');$('cwd').value=statusData.cwd||'';const proto=location.protocol==='https:'?'wss:':'ws:';ws=new WebSocket(proto+'//'+location.host+'/api/codex/ws?token='+encodeURIComponent(t));ws.onmessage=onMessage;ws.onclose=()=>{$('connection').textContent='Disconnected';$('connection').className='status bad'};await new Promise((resolve,reject)=>{ws.onopen=resolve;ws.onerror=()=>reject(new Error('WebSocket connection failed'))});await request('initialize',{clientInfo:{name:'trebell-mobile',title:'Trebell Remote',version:statusData.version},capabilities:{experimentalApi:true,mcpServerOpenaiFormElicitation:true,extensions:{'openai/form':{},'openai/elicitation':{form:{}}}}});ws.send(JSON.stringify({method:'initialized',params:{}}));$('connection').textContent='Connected';$('connection').className='status ok';await Promise.all([hasScope('threads:read')?loadThreads():Promise.resolve(),hasScope('environments:read')?loadEnvironments():Promise.resolve(),hasScope('threads:write')?loadCollaborationModes():Promise.resolve()])}",
    "function collaborationModePayload(){const selected=collaborationModes.find(item=>item.mode===$('collaborationMode').value);if(!selected)return null;const model=selected.model||statusData?.model||'';if(!model)return null;return {mode:selected.mode,settings:{model,reasoning_effort:selected.reasoning_effort??null,developer_instructions:null}}}",
    "async function loadCollaborationModes(){const select=$('collaborationMode');collaborationModes=[];select.innerHTML='';select.hidden=true;if(statusData?.agentRuntime!=='codex')return;const result=await request('collaborationMode/list',{}).catch(()=>({data:[]}));collaborationModes=result.data||[];for(const mode of collaborationModes){if(!mode?.mode)continue;const option=document.createElement('option');option.value=mode.mode;option.textContent=(mode.name||mode.mode)+' mode';select.appendChild(option)}if(select.options.length){select.hidden=false;if(collaborationModes.some(mode=>mode.mode==='default'))select.value='default'}}",
