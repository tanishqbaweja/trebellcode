...
function lanUrls(port,token){
  ...
  for(const entries of Object.values(networkInterfaces())){
    for(const info of entries||[]){
      ...
      const url="http://"+info.address+":"+port+"/#token="+encodeURIComponent(token);
...
function mobileHtml(){
  return [
    ...
    "<section class=\"card\"><div class=\"row stack\"><input id=\"token\" class=\"grow\" type=\"password\" placeholder=\"Access token\"><button id=\"connect\" class=\"primary\">Connect</button></div><div id=\"status\" class=\"status\"></div></section>",
    ...
    "const hash=new URLSearchParams(location.hash.replace(/^#/,''));const saved=hash.get('token')||localStorage.getItem('trebellRemoteToken')||'';$('token').value=saved;if(hash.get('token'))history.replaceState(null,'',location.pathname);",
    ...
    "function approvalResult(msg,accept){if(msg.method==='item/permissions/requestApproval')return {permissions:msg.params?.permissions||{},scope:'turn'};return {decision:accept?'accept':'decline'}}",
    "function handleServerRequest(msg){if(msg.method==='item/tool/requestUserInput'){const answers={};for(const q of msg.params?.questions||[]){const a=prompt(q.question||q.header||'Trebell needs input','')||'';answers[q.id]={answers:a?[a]:[]}}respond(msg.id,{answers});return}if(msg.method.includes('requestApproval')||msg.method==='applyPatchApproval'||msg.method==='execCommandApproval'){const ok=confirm(msg.params?.reason||msg.params?.command||msg.params?.path||'Allow this Trebell action?');respond(msg.id,approvalResult(msg,ok));return}respond(msg.id,null,{code:-32601,message:'Unsupported mobile request: '+msg.method})}",
...
export async function createRemoteControlServer({
  ...
  const server=createServer(async(req,res)=>{
    ...
    if(!sameToken(authToken(req,url),token)) return json(res,401,{error:"unauthorized"});
    try{
  ...
  const relay=attachCodexRelay(server,{
    ...
    enabled,
    authorize:(request,url)=>sameToken(authToken(request,url),token),
  });
