...
export class RemoteAuthStore{
  ...
  #load(){try{const parsed=JSON.parse(readFileSync(this.path,"utf8"));return {version:1,pairings:Array.isArray(parsed.pairings)?parsed.pairings:[],devices:Array.isArray(parsed.devices)?parsed.devices:[]}}catch{return {version:1,pairings:[],devices:[]}}}
  ...
  createPairing({ttlMs=10*60_000}={}){
    this.#purge();const raw=token("pair");const item={id:randomBytes(8).toString("hex"),tokenHash:digest(raw),createdAt:Date.now(),expiresAt:Date.now()+Math.max(60_000,Math.min(60*60_000,Number(ttlMs)||10*60_000)),usedAt:null};
    this.data.pairings.push(item);this.#save();return {id:item.id,token:raw,createdAt:item.createdAt,expiresAt:item.expiresAt};
  }
  exchangePairing(raw,{name="Remote device",userAgent=""}={}){
    ...
    grant.usedAt=Date.now();const sessionToken=token("device");const device={id:randomBytes(10).toString("hex"),name:String(name||"Remote device").trim().slice(0,120)||"Remote device",tokenHash:digest(sessionToken),createdAt:Date.now(),lastSeenAt:Date.now(),userAgent:String(userAgent||"").slice(0,300)};
