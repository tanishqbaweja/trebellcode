...
const DIAGNOSTIC_SOURCE=/\.(?:[cm]?[jt]sx?|pyi?|go)$/i;
...
export async function runAutomaticVerificationEvidence({contextEngine,plan,evidence=[],root,io=null,limit=100}={}){
  ...
  if(!targets.length){
    const update={stepId:step.id,status:"blocked",reason:"No changed JavaScript/TypeScript/Python/Go source paths were available for deterministic diagnostics.",source:"harness-diagnostics",coveredPaths:[]};
