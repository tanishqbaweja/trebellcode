...
export class NativeAgentSession{
  constructor({cwd=process.cwd(),providerTurn,executeTool,provider=null,model=null,tools=[],permissionMode="supervised",onUpdate=()=>{},initialMessages=[]}={}){
  ...
  async prompt(prompt,{messageId=null,maxModelTurns=24,maxToolCalls=100}={}){
    ...
    try{
      const result=await runNativeAgentTurn({
        provider:this.provider,model:this.model,messages:base,tools:this.tools,maxModelTurns,maxToolCalls,signal:this.controller.signal,
