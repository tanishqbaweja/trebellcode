...
export const SHARED_TOOL_NAMESPACE_CATALOG=freeze([
  ...
  namespace("trebell_device","Inspect local Android emulators and iOS simulators. Android supports input controls; iOS currently supports screenshots plus boot/power-off lifecycle controls. Physical phones are never controlled by these tools.",[
    ...
    tool("packages","List bounded third-party package ids installed in an Android emulator.",{type:"object",properties:{id:{type:"string"}},required:["id"],additionalProperties:false},{kind:"read",riskLevel:"low",reversibility:"not-applicable",idempotent:true,asyncSafe:true},{deviceAccess:true}),
    ...
    tool("tap","Tap Android emulator pixel coordinates from the latest screenshot.",{type:"object",properties:{id:{type:"string"},x:{type:"number"},y:{type:"number"}},required:["id","x","y"],additionalProperties:false},{kind:"other",riskLevel:"medium",reversibility:"partial"},{deviceAccess:true}),
