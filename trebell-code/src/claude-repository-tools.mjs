...
export function repositoryToolHandlers({contextEngine,root,io=null}={}){
  ...
  return {
    ...
    diagnostics:({path,limit=100})=>contextEngine.diagnostics({root,io,path,limit}),
...
export function createClaudeRepositoryMcp({contextEngine,root,io=null,version="0.0.0"}={}){
  ...
  return createSdkMcpServer({
    ...
    tools:[
      ...
      tool("diagnostics","Parse an indexed JavaScript or TypeScript source file and return bounded syntax diagnostics. This is parser evidence, not type/LSP semantics.",{path:z.string().min(1),limit:z.number().int().min(1).max(200).optional()},async args=>({content:[{type:"text",text:JSON.stringify(await handlers.diagnostics(args))}]}),{searchHint:"repository diagnostics syntax parser errors javascript typescript"}),
