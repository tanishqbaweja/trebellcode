...
export function createClaudeRepositoryMcp({contextEngine,root,io=null,version="0.0.0"}={}){
  ...
  return createSdkMcpServer({
    ...
    tools:[
      ...
      tool("project_commands","Discover bounded repository-declared build, test, lint, typecheck, format, dev, and start commands plus clearly labeled ecosystem conventions.",{limit:z.number().int().min(1).max(240).optional()},async args=>({content:[{type:"text",text:JSON.stringify(await handlers.projectCommands(args))}]}),{searchHint:"repository project commands scripts build test lint typecheck package manager make just cargo go pytest maven gradle"}),
      tool("verification_plan","Build a deterministic risk-aware verification plan from changed or explicit paths, related tests, and discovered project commands. This plans evidence only; it does not execute commands or spawn reviewer models.",{paths:z.array(z.string().min(1)).max(200).optional(),riskHints:z.array(z.string().min(1).max(80)).max(20).optional(),capabilities:z.object({diagnostics:z.boolean().optional(),semanticDiagnostics:z.boolean().optional()}).optional()},async args=>({content:[{type:"text",text:JSON.stringify(await handlers.verificationPlan(args))}]}),{searchHint:"verification plan changed files risk tests build browser screenshot integration evidence"}),
