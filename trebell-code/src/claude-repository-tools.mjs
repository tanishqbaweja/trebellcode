...
export function createClaudeRepositoryMcp({contextEngine,root,io=null,version="0.0.0"}={}){
  ...
  return createSdkMcpServer({
    ...
    tools:[
      ...
      repositoryTool("verification_next","Choose the next deterministic verification action from a plan and recorded evidence: verify the cheapest missing required step, repair a failure, resolve a blocker, or complete. Independent review remains advisory.",{plan:z.record(z.string(),z.unknown()),evidence:z.array(z.record(z.string(),z.unknown())).max(300).optional()},async args=>({content:[{type:"text",text:JSON.stringify(await handlers.verificationNext(args))}]}),{searchHint:"verification next action repair blocker cheapest remaining step complete same agent loop"}),
