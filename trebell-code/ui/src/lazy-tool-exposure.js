...
const DEVICE_TASK=/\b(android|ios|iphone|ipad|emulator|simulator|device screenshot|mobile app)\b/i;
...
export function specializedToolSelection(taskText,{browser=false,computer=false,device=false,sourceControl=false,delegation=false}={}){
  const task=text(taskText);
  return {
    browser:Boolean(browser&&BROWSER_TASK.test(task)),
    computer:Boolean(computer&&COMPUTER_TASK.test(task)),
    device:Boolean(device&&DEVICE_TASK.test(task)),
