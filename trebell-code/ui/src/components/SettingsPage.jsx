...
export default function SettingsPage({settings,onSettings,onProviderChanging,onProviderUpdated,runtime,runtimeCapabilities={},rpcStatus,loggedIn,login,logout,projectPath,runtimeEnvironmentId=null,onOpenRuntimeAuthTerminal,projectScripts=[],modelError,onOpenLicenses,models=[],onScopedSettingsChanged,environmentThemeCatalog={environmentKey:"local",environmentName:"Local machine",directory:"",themes:[]},environmentThemes=[],onRefreshEnvironmentThemes}){
  ...
  async function addCustomModel(){
    ...
    const provider=["native","codex"].includes(selectedAgent)?selected:null;
    const entry={
    ...
    const next=[...current.filter(item=>!(item.id===id&&item.runtime===selectedAgent&&(!["native","codex"].includes(selectedAgent)||item.provider===selected))),entry];
  ...
  async function refresh({
    ...
    includeStorage=true,
    includeDesktopUpdates=true,
    includeSnapshots=true,
    includeBrowser=true,
  }={}){
    setLoading(true);if(reportErrors)setSettingsError("");
    const checks=[];
    if(includeUpdate)checks.push(["update",api("/api/update/check")]);
    if(includeDiagnostics)checks.push(["diagnostics",api("/api/diagnostics?path="+encodeURIComponent(projectPath||""))]);
    if(includeStorage)checks.push(["storage",loadStorageInfo({strict:reportErrors})]);
    if(includeRuntime)checks.push(["providers",loadProviders({strict:reportErrors})],["runtimes",loadAgentRuntimes({strict:reportErrors})]);
    ...
    setLoading(false);
    if(includeDesktopUpdates&&window.trebellDesktop?.updates)window.trebellDesktop.updates.get().then(info=>{setDesktopUpdate(info);setSettingsError(current=>/^Could not load desktop update state:/.test(current)?"":current)}).catch(error=>setSettingsError("Could not load desktop update state: "+(error?.message||String(error))));
    if(includeSnapshots&&window.trebellDesktop?.snapshots)window.trebellDesktop.snapshots.get().then(info=>{setSnapshotInfo(info);setSnapshotMessage(current=>/^Could not load SnapShot settings:/.test(current)?"":current)}).catch(error=>setSnapshotMessage("Could not load SnapShot settings: "+(error?.message||String(error))));
  ...
  useEffect(()=>{
    if(settingsSection==="general")refresh({includeRuntime:false,includeDiagnostics:false,includeStorage:false,includeSnapshots:false,includeBrowser:false});
    else if(settingsSection==="workspace")refresh({includeRuntime:false,includeUpdate:false,includeDiagnostics:false,includeDesktopUpdates:false,includeSnapshots:false,includeBrowser:false});
  ...
  return <div className="settings-page redesigned-settings">
    ...
    <section className="settings-stage">
      ...
      {settingsSection==="general"&&<div className="settings-card about-card" {...targetProps("general-about")}>
      ...
      {settingsSection==="agents"&&["native","codex"].includes(selectedAgent)&&<div className="settings-card provider-settings-card" {...targetProps("agents-provider")} data-testid="provider-settings-card" aria-busy={providerSwitching?"true":"false"}>
      ...
      {settingsSection==="desktop"&&window.trebellDesktop?.background&&<div className="settings-card"><h3>Background mode</h3><p>Keep Trebell's local harness running in the system tray after the window closes, and start it with Windows.</p><label className="toggle-line"><input type="checkbox" checked={Boolean(settings.backgroundMode)} onChange={e=>setBackgroundMode(e.target.checked)}/> Keep Trebell running in background</label></div>}
