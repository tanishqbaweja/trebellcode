...
export default function SettingsPage({settings,onSettings,onProviderChanging,onProviderUpdated,runtime,rpcStatus,loggedIn,login,logout,projectPath,runtimeEnvironmentId=null,onOpenRuntimeAuthTerminal,projectScripts=[],modelError,onOpenLicenses,models=[],onScopedSettingsChanged,environmentThemeCatalog={environmentKey:"local",environmentName:"Local machine",directory:"",themes:[]},environmentThemes=[],onRefreshEnvironmentThemes}){
  ...
  async function save(patch){
    ...
    catch(error){if(optimistic){onProviderChanging?.(settings.modelProvider||"freebuff");onSettings(settings)}if(providerChange)setProviderSwitching(false);throw error}
    onSettings(next);
  ...
  const settingsSections=[
    ...
    ["desktop",MonitorCog,"Desktop","Computer use, browser profiles and SnapShots"],
  ...
  return <div className="settings-page redesigned-settings">
    ...
    <section className="settings-stage">
      ...
      <div className="settings-card" hidden={settingsSection!=="desktop"}><h3>Background mode</h3><p>Keep Trebell's local harness running in the system tray after the window closes, and start it with Windows.</p><label className="toggle-line"><input type="checkbox" checked={Boolean(settings.backgroundMode)} onChange={async e=>{const enabled=e.target.checked;await window.trebellDesktop?.background?.set?.(enabled);await save({backgroundMode:enabled})}}/> Keep Trebell running in background</label></div>
    ...
    <div className="diagnostics-log" hidden={settingsSection!=="diagnostics"}><div><strong>Runtime log</strong><button onClick={refresh} disabled={loading}><RefreshCw size={12}/></button></div><pre>{(diagnostics?.logs||[]).map(x=>"["+new Date(x.at).toLocaleTimeString()+"] "+x.stream+": "+x.text).join("")||"No runtime log entries."}</pre></div>
    </section>
  </div>;
}
