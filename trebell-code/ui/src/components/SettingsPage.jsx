...
export default function SettingsPage({settings,onSettings,onProviderUpdated,runtime,rpcStatus,loggedIn,login,logout,projectPath,modelError,onOpenLicenses}){
  ...
  async function setGlobalCleanupMode(mode){
    if(mode==="off")return save({worktreeCleanup:{mode:"off"}});
    const current=settings.worktreeCleanup?.mode==="custom"?settings.worktreeCleanup.rules:{};
    return save({worktreeCleanup:{mode:"custom",rules:{worktreeAfterDays:current?.worktreeAfterDays??30,worktreeOnMerge:Boolean(current?.worktreeOnMerge),worktreeOnDelete:Boolean(current?.worktreeOnDelete),worktreeUnchanged:Boolean(current?.worktreeUnchanged)}}});
  }
  async function setGlobalCleanupRule(key,value){
    const current=settings.worktreeCleanup?.mode==="custom"?settings.worktreeCleanup.rules:{};
    return save({worktreeCleanup:{mode:"custom",rules:{worktreeAfterDays:current?.worktreeAfterDays??30,worktreeOnMerge:Boolean(current?.worktreeOnMerge),worktreeOnDelete:Boolean(current?.worktreeOnDelete),worktreeUnchanged:Boolean(current?.worktreeUnchanged),[key]:value}}});
  }
  async function runWorktreeCleanup(){
    setCleanupMessage("Running cleanup…");
    try{const result=await api("/api/worktree/cleanup",{method:"POST",body:{}});setCleanupMessage(result.removed?`Removed ${result.removed} safe managed worktree${result.removed===1?"":"s"}.`:"No managed worktrees were eligible for cleanup.")}
    catch(error){setCleanupMessage("Cleanup failed: "+(error.message||String(error)))}
  }
