...
export default function LicensesPage(){
  ...
  return <div className="licenses-page">
    ...
    <div className="licenses-layout"><div className="licenses-list">{filtered.map(item=><button key={item.id} className={selected===item.id?"active":""} onClick={()=>open(item)}><FileText size={13}/><span><strong>{item.name}</strong><small>{item.version||"unknown version"} · {item.license||"Unknown"}</small></span><em>{item.component}</em></button>)}{!filtered.length&&<p>No matching licenses.</p>}</div>
      <div className="license-detail">{detail?<><div className="license-detail-head"><div><strong>{detail.name}</strong><span>{detail.version} · {detail.license}</span></div><button onClick={()=>{setSelected(null);setDetail(null)}}><X size={13}/></button></div>{detail.homepage&&<p>{detail.homepage}</p>}<pre>{detail.text}</pre></>:<div className="empty-state">Select a package to read its installed license notice.</div>}{error&&<div className="inline-error">{error}</div>}</div>
    </div>
  </div>;
}
