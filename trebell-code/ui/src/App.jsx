...
const Composer=memo(function Composer({prompt,setPrompt,onPromptEdit,historyIndex=-1,onSend,onBackgroundSend,canBackground=false,running,submitting=false,providerReady,provider,agentRuntime="codex",agentRuntimeLabel="Codex",runtimeCapabilities={},login,onConfigureProvider,models,modelMeta,model,setModel,selectedModels=[],onSelectedModels,allowMultiModel=false,modelError,freebuff,attachments,contextChips,onRemoveAttachment,onRemoveContext,onPickFiles,onCaptureScreen,onPaste,onDrop,onFileMentionSearch,onFileMentionAttach,permissionMode,setPermissionMode,collaborationModes=[],collaborationMode="default",onCollaborationMode,collaborationModeBusy=false,providerCommands=[],providerAgents=[],providerAgent="",onProviderAgent,settings,tokenUsage,workspaceMode,setWorkspaceMode,projectless=false,threadOpen=false,gitAvailable=false,canCompact=false,onCompact,runtimeProfiles=null,runtimeProfileBusy="",onRuntimeProfile,onModelPickerOpenChange}){
  ...
  async function chooseMention(item){
    ...
    setMentionBusy(true);
    try{
      const attached=await onFileMentionAttach?.(item);
      if(!attached)return;
...
export default function App(){
  ...
  async function refreshEnvironmentThemes({strict=false}={}){
    try{
      const catalog=await api("/api/environment/themes");
      setEnvironmentThemeCatalog(catalog);
      return catalog;
    }catch(error){
      if(strict)throw error;
      return environmentThemeCatalog;
    }
  }
  ...
  async function switchThreadRuntimeProfile(instanceId){
    ...
    try{
      ...
      showActionError(error,`Could not switch ${label} profile`);
      return false;
  ...
  async function loadSkills(client,path=projectPath,forceReload=false,{strict=false}={}){
    ...
    try{
      ...
      const next=(result.data||[]).flatMap(x=>x.skills||[]).map(skill=>{
        const skillPath=skill?.path;if(!skillPath||!skillOverridesRef.current.has(skillPath))return skill;
        const confirmedEnabled=Boolean(skillOverridesRef.current.get(skillPath));
        if((skill.enabled!==false)===confirmedEnabled){skillOverridesRef.current.delete(skillPath);return skill}
        return {...skill,enabled:confirmedEnabled};
      });
  ...
  async function refreshSkillsAfterMutation(fallback){
    if(fallback?.path){
      const enabled=Boolean(fallback.enabled);
      skillOverridesRef.current.set(fallback.path,enabled);
      setSkills(current=>current.map(skill=>skill.path===fallback.path?{...skill,enabled}:skill));
    }
    return loadSkills(rpc,projectPath,true,{strict:true});
  }
  ...
  async function sendInBackground(){
    if(!runtimeCapabilities.backgroundProcesses){await send();return}
  ...
  async function uploadBrowserFiles(files){
    const uploaded=[];let firstError=null;
    for(const file of files.slice(0,Math.max(0,MAX_COMPOSER_ATTACHMENTS-attachments.length))){
      try{uploaded.push((await blobAttachment(file)).path)}
      catch(error){firstError ||= error}
    }
    await addFiles(uploaded);
