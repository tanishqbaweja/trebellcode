...
test("Trebell Native keeps Freebuff account surfaces available when Freebuff supplies inference",async({page})=>{
  ...
  const project={id:"native-freebuff-project",name:"Native Freebuff Project",path:repo,environmentId:null,effectiveSettings:{}};let loggedIn=false,loginStarts=0,overviewCalls=0,heartbeatCalls=0,uiMeta={projectless:false,environmentId:null,cwd:repo,runtime:"native",runtimeInstanceId:"native-default"};
  ...
  try{
    await page.route(/\/api\/bootstrap$/,route=>route.fulfill({status:200,contentType:"application/json",body:JSON.stringify({mock:false,loggedIn,provider:"freebuff",providerReady:loggedIn,agentRuntime:"native",agentRuntimeReady:true,wsUrl:`ws://127.0.0.1:${relayPort}/api/agent/ws`,cwd:repo,platform:process.platform,version:"visual-fixture",activeEnvironmentId:null,activeEnvironment:null})}));
    ...
    await page.route(/\/api\/models$/,route=>route.fulfill({status:200,contentType:"application/json",body:JSON.stringify({provider:"freebuff",agentRuntime:"native",ready:loggedIn,models:loggedIn?[model]:[],metadata:{provider:"freebuff",models:loggedIn?[{id:model,name:"Coding Fast",provider:"freebuff",agent:"Trebell Native"}]:[]}})}));
    await page.route(/\/api\/login\/start$/,route=>{loginStarts++;loggedIn=true;return route.fulfill({status:202,contentType:"application/json",body:JSON.stringify({started:true})})});
    ...
    await page.goto("/");const providerButton=page.locator(".sidebar-provider");await expect(providerButton).toContainText("Trebell Native");await expect(providerButton).toContainText("Freebuff inference");
    const composer=page.getByTestId("composer");await expect(composer).toHaveAttribute("placeholder","Sign in to Freebuff to start…");const loginButton=page.getByRole("button",{name:"Sign in to Freebuff",exact:true});await expect(loginButton).toBeVisible();await loginButton.click();await expect.poll(()=>loginStarts).toBe(1);await expect(page.getByTestId("model-picker")).toContainText("Coding Fast",{timeout:10_000});await expect.poll(()=>overviewCalls,{timeout:10_000}).toBeGreaterThan(0);
