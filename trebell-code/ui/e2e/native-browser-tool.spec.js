...
test("Trebell Native receives a desktop-browser screenshot as a model-visible image",async({page})=>{
  ...
  try{
    await page.addInitScript(({image})=>{window.__browserScreenshotCalls=0;window.trebellDesktop={browser:{screenshot:async()=>{window.__browserScreenshotCalls++;return{dataUrl:image,url:"https://example.test",title:"Fixture page"}}}}},{image:IMAGE_DATA_URL});
    ...
    const composer=page.getByTestId("composer");await composer.fill("Take a browser screenshot");await page.getByTestId("send").click();await expect(page.getByText("Browser screenshot received.",{exact:true})).toBeVisible({timeout:10_000});expect(await page.evaluate(()=>window.__browserScreenshotCalls)).toBe(1);
