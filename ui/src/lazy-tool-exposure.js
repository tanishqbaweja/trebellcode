...
const PROCESS_TASK=/\b(background|long[- ]running|dev server|development server|server process|watch(?:er| mode)?|daemon|keep (?:it|this) running|run .*server)\b/i;

export function specializedToolSelection(taskText,{browser=false,computer=false,sourceControl=false,delegation=false,process=false}={}){
  ...
  return {
    ...
    process:Boolean(process&&PROCESS_TASK.test(task)),
  };
}
...
export function specializedToolNamespaceNames(taskText,availability={}){
  ...
  if(selected.process)names.push("trebell_process");
  return names;
}
