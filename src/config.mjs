...
const PROVIDERS = {
  ...
  agentrouter: { name: "Trebell AgentRouter", baseUrl: (port) => `http://127.0.0.1:${port}/v1` },
  justworker: { name: "Trebell JustWorker", baseUrl: (port) => `http://127.0.0.1:${port}/v1` },
  hcnsec: { name: "Trebell HCNSec", baseUrl: (port) => `http://127.0.0.1:${port}/v1` },
  vyceai: { name: "Trebell VyceAi", baseUrl: (port) => `http://127.0.0.1:${port}/v1` },
};

function defaultProviderPort(provider){ return provider==="freebuff"?DEFAULT_PORT:PROVIDER_COMPAT_PORT; }

export function codexProviderOverrides({port=null,provider="freebuff"}={}){
  const selectedId=provider in PROVIDERS?provider:"freebuff";
  const selected=PROVIDERS[selectedId];
  const selectedPort=Number.isInteger(port)?port:defaultProviderPort(selectedId);
  return [
    "--config",`model_provider=${JSON.stringify(selectedId)}`,
    "--config",`model_providers.${selectedId}.name=${JSON.stringify(selected.name)}`,
    "--config",`model_providers.${selectedId}.base_url=${JSON.stringify(selected.baseUrl(selectedPort))}`,
...
export function renderCodexConfig({ port = null, provider = "freebuff" } = {}) {
  const selectedId = provider in PROVIDERS ? provider : "freebuff";
  const selected = PROVIDERS[selectedId];
  const selectedPort=Number.isInteger(port)?port:defaultProviderPort(selectedId);
...
name = "${selected.name}"
base_url = "${selected.baseUrl(selectedPort)}"
wire_api = "responses"
...
export function ensureCodexConfig({ port = null, provider = "freebuff", env = process.env } = {}) {
  ensureDirs(env);
