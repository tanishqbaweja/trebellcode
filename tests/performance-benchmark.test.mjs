...
import { benchmarkAgentThreadSearch, benchmarkConversationVirtualization, benchmarkDurableState, benchmarkReplay, benchmarkRepositoryIndex, benchmarkStreamingCoalescing, runCoreBenchmark, syntheticEventHistory } from "../src/performance-benchmark.mjs";
...
test("agent thread benchmark measures SQLite conversation search without hydrating full histories",async()=>{
...
test("core benchmark exercises SQLite event storage without a hard machine-speed threshold",async()=>{
  ...
  try{
    const result=await runCoreBenchmark({env:{TREBELL_HOME:home,HOME:home},threads:10,eventsPerThread:6,eventCount:200,queryCount:10,stateRecords:30,threadSearchThreads:30,repoFiles:30,longChatMessages:500,streamDeltas:1000});
